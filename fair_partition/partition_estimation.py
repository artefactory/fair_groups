...
class FairGroups:
    ...
    def fit(self, s, y):
        ...
        num_combinations = sum(
            1 for _ in itertools.combinations(internal_points, self.nb_groups - 1)
        )

        # Select (nb_groups - 1) internal split points to form nb_groups intervals
        for internal_split in tqdm(
            itertools.combinations(internal_points, self.nb_groups - 1), total=num_combinations
        ):
            ...
            phi_by_group_tmp = np.array(
                [phi_dict[s_values[i], s_values[i + 1]] for i in range(self.nb_groups)]
            )
